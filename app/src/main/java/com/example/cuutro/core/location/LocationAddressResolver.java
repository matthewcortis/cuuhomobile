...
public class LocationAddressResolver {
    ...
    public LocationAddressData reverseGeocode(@NonNull LatLng target, @NonNull String fallbackAddress) {
        ...
        try {
            ...
            String city = extractProvinceOrCity(address, fullAddress);
            String postalCode = extractWardOrCommune(address, fullAddress);
    ...
    @NonNull
    private String extractProvinceOrCity(@NonNull Address address, @NonNull String fullAddress) {
        String provinceOrCity = firstNonBlank(address.getAdminArea());
        if (!provinceOrCity.isBlank()) {
            return provinceOrCity;
        }

        provinceOrCity = firstNonBlank(
                findSegmentByPrefix(fullAddress, "thanh pho", "tinh"),
                address.getLocality(),
    ...
    @NonNull
    private String extractWardOrCommune(@NonNull Address address, @NonNull String fullAddress) {
        String wardOrCommune = firstNonBlank(address.getSubLocality());
        if (!wardOrCommune.isBlank()) {
            return wardOrCommune;
        }

        wardOrCommune = firstNonBlank(
                findSegmentByPrefix(fullAddress, "phuong", "xa", "thi tran"),
                address.getSubAdminArea(),
                splitAddressSegment(fullAddress, 1),
    ...
    @NonNull
    private String findSegmentByPrefix(@NonNull String fullAddress, String... prefixes) {
        if (fullAddress.isBlank() || prefixes == null || prefixes.length == 0) {
            return "";
        }

        String[] segments = fullAddress.split(",");
        for (String segment : segments) {
            ...
            String normalized = normalizeVietnameseText(value);
    ...
    @NonNull
    private String normalizeVietnameseText(@NonNull String input) {
